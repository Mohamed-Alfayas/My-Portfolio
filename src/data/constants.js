import linthrImg from "../images/ProjectImages/lintHr.jpg";
import cackeryImg from "../images/ProjectImages/cackery.jpg";
import postManImg from "../images/ProjectImages/postman-icon.svg";
import posImg from "../images/ProjectImages/pos.jpg";
import lcCare from "../images/ProjectImages/lc_care.png";
import cakeyDashboard from "../images/ProjectImages/cakery_dashboard.jpg";
import JavaScriptIcon from "../images/icon/java-script.png";
import CssIcon from "../images/icon/css.png";
import VsCodeIcon from "../images/icon/vs-code.png";
import MissFishDashboard from "../images/ProjectImages/miss-fish-dashboard.png";
import MissFishWebsite from "../images/ProjectImages/miss-fish-website.png";

export const Bio = {
  name: "Mohamed Alfayas",
  roles: ["Front-End Web Developer"],
  description:
    "I am a motivated and versatile individual, always eager to take on new challenges. With a passion for learning I am dedicated to delivering high-quality results. With a positive attitude and a growth mindset, I am ready to make a meaningful contribution and achieve great things.",
  github: "https://github.com/Mohamed-Alfayas",
  resume:
    "https://drive.google.com/file/d/1eo_yhoISAwRpfzYEtUkWaMvV_YyNblU0/view?usp=sharing",
  linkedin: "https://www.linkedin.com/in/mohamed-alfayas-5a2487227/",
};

export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
      {
        name: "Redux",
        image:
          "https://d33wubrfki0l68.cloudfront.net/0834d0215db51e91525a25acf97433051f280f2f/c30f5/img/redux.svg",
      },

      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image: CssIcon,
      },
      {
        name: "JavaScript",
        image: JavaScriptIcon,
      },
      {
        name: "Bootstrap",
        image:
          "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      },
      {
        name: "Material UI",
        image:
          "data:image/png;base64,",
      },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "PHP",
        image: "https://www.php.net/images/logos/php-logo-white.svg",
      },

      {
        name: "MySQL",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg",
      },

      {
        name: "Firebase",
        image: "https://www.vectorlogo.zone/logos/firebase/firebase-icon.svg",
      },
    ],
  },

  {
    title: "Others",
    skills: [
      {
        name: "GitHub",
        image:
          "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
      },
      {
        name: "Docker",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original-wordmark.svg",
      },

      {
        name: "VS Code",
        image: VsCodeIcon,
      },
      {
        name: "Postman",
        image: postManImg,
      },
    ],
  },
];

export const experiences = [
  {
    id: 0,
    img: "",
    role: "Front-End Web Developer",
    company: "LINTCLOUD",
    date: "Jul 2023 - Present",
    desc: "Designed and implemented userinterfaces and functionality for various projects, including an employee attendance system, customer support platform, and monitoring dashboard, using React, Redux Toolkit, JavaScript, HTML, CSS, and jQuery.",
    skills: [
      "ReactJS",
      "Redux",
      "Material UI",
      "HTML",
      "CSS",
      "Bootstrap",
      "JavaScript",
      "Docker",
      "Git Hub",
    ],
  },
  {
    id: 1,
    img: "",
    role: "Web Developer Intern",
    company: "LINTCLOUD",
    date: " Jan 2023 - Jun 2023",
    desc: "Followed HTML and CSS best practices to ensure accessibility and usability, while assisting in cross-browsertesting to ensure a consistent experience across multiple platforms.",
    skills: ["HTML", "CSS", "Bootstrap", "JavaScript", "ReactJS"],
    doc: "",
  },
];

export const education = [
  {
    id: 0,
    img: "",
    school: "PET Engineering College",
    date: "Sep 2018 - Jul 2022",
    grade: "7.7 CGPA",
    desc: "I Have completed My Bachelor's degree in Mechanical Engineering at PET Engineering College, Vallioor, Tirunelveli Dist. ",
    degree: "Bachelor of Engineering - BE, Mechanical Engineering",
  },
  {
    id: 1,
    img: "",
    school: "Sacred Heart Hr Sec School",
    date: "Jun 2017 - Apr 2018",
    grade: "65.41%",
    desc: "I Have completed my class 12 high school education at Sacred Heart Hr Sec School,Panagudi, where I studied Maths with Computer Science.",
    degree: "HSC(XII),  Computer Science Maths",
  },
  {
    id: 2,
    img: "",
    school: "Sacred Heart Hr Sec School",
    date: "Jun 2015 - Apr 2016",
    grade: "70.4%",
    desc: "I Have completed my class 10 education at Sacred Heart Hr Sec School,Panagudi.",
    degree: "SSLC(X)",
  },
];

export const projects = [
  {
    id: 0,
    title: "LintHR",
    date: "Sep 2023 - Present",
    description:
      "LintHr is a modern web application tailored for organizations to streamline employee attendance and leave management processes,Automates manual attendance tracking and leave approvals. saving time and effort. Ensures employees and employers have a clear view of attendance and leave records.",
    image: linthrImg,
    tags: ["Docker", "AWS", "React Js", "Postgresql", "Django", "Redux"],
    category: "web app",
  },

  {
    id: 1,
    title: "MagilShe.Cakery",
    date: "Jan 2024 - Sep 2024",
    description:
      "Magilshe Cakery is a beautifully designed web application dedicated to cake enthusiasts, offering an easy and delightful way to explore, customize, and order cakes for various occasions. Built using React, this web app combines an intuitive user interface with robust functionality to provide a seamless online cake ordering experience. Whether it’s for a birthday, wedding, anniversary, or any special event, Magilshe Cakery is the perfect platform to satisfy your sweet tooth.",
    image: cackeryImg,
    tags: ["AWS", "React Js", "Postgresql", "Django", "Redux"],
    category: "web app",
  },
  {
    id: 2,
    title: "MagilShe.Cakery - Dashboard",
    date: "March 2024 - Oct 2024",
    description:
      "MagilShe.Cakery - Dashboard is a feature-rich and elegantly crafted web application tailored for the administrative needs of cake businesses. Built with React, this dashboard serves as a powerful backend solution for managing the operations of MagilShe Cakery, ensuring seamless control and oversight of orders, customers, inventory, and more.The dashboard offers an intuitive interface and advanced functionalities that enable admins to efficiently handle the workflow. From managing cake menus, customizing cake designs, and tracking order statuses to monitoring sales and generating insightful reports, MagilShe.Cakery - Dashboard is designed to optimize business processes.",
    image: cakeyDashboard,
    tags: ["AWS", "React Js", "Postgresql", "Django", "Redux"],
    category: "web app",
  },
  {
    id: 3,
    title: "Missfish Website",
    date: "March 2025 - March 2026",
    description:
      "Missfish is a modern eCommerce platform developed for selling fresh fish online with a seamless shopping experience. The application enables customers to browse a wide range of seafood products, manage their cart, place orders, and securely complete payments using Razorpay. Built with React and Redux Toolkit Query (RTK Query), the platform delivers efficient API data fetching, caching, and state management for a fast and responsive user experience. The backend is powered by Django with PostgreSQL, while AWS is used for reliable hosting and scalability, ensuring a secure and high-performance online marketplace.",
    image: MissFishWebsite,
    tags: [
      "AWS",
      "React Js",
      "PostgreSQL",
      "Django",
      "Redux Toolkit",
      "RTK Query",
      "Razorpay",
    ],
    category: "web app",
  },
  {
    id: 4,
    title: "Missfish- Dashboard",
    date: "March 2024 - Oct 2024",
    description:
      "Missfish Dashboard is a comprehensive admin panel designed to efficiently manage the entire fish eCommerce platform. Built with React and Redux Toolkit Query (RTK Query), it provides a fast and responsive interface for managing products, categories, customer orders, inventory, users, and payments. Administrators can monitor sales, update order statuses, manage deliveries, and generate business reports through an intuitive dashboard. Powered by Django and PostgreSQL, with AWS for deployment, the system ensures secure, scalable, and reliable management of day-to-day eCommerce operations.",
    image: MissFishDashboard,
    tags: [
      "AWS",
      "React Js",
      "PostgreSQL",
      "Django",
      "Redux Toolkit",
      "RTK Query",
    ],
    category: "web app",
  },
  {
    id: 5,
    title: "LC CARE",
    date: "Jul 2023 - Dec 2023",
    description:
      "LC Care is a powerful and efficient Hospital Management Software designed to simplify and optimize healthcare operations. Built with JavaScript, jQuery, and Django, LC Care caters to the needs of hospitals, clinics, and other healthcare facilities by offering tools for managing patients, appointments, medical records, and staff operations. With a focus on reliability, scalability, and user-friendliness, LC Care ensures that healthcare providers can focus on delivering exceptional care.",
    image: lcCare,
    tags: ["Javascript/Jquery", "Ajax", "Bootstrap", "HTML,CSS", "Django"],
    category: "web app",
  },
  {
    id: 6,
    title: "LC POS",
    date: "Jul 2023 - Nov 2023",
    description:
      "LC POS web application is a comprehensive, PHP-based solution designed to streamline retail operations, enhance efficiency, and deliver exceptional customer experiences. This system empowers businesses to handle sales, inventory, and customer management with ease, making it an essential tool for retail stores, restaurants, Medical and other service providers.",
    image: posImg,
    tags: ["PHP", "SQL", "Jqurey", "Xampp", "PHPmyadmin"],
    category: "web app",
  },
];
